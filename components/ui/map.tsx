...
export function WorldMap({
  ...
  const svgMap = useMemo(
    () =>
      map.getSVG({
        radius: 0.22,
        color: theme === "dark" ? "#FFFFFF55" : "#FFFFFF40",
        shape: "circle",
  ...
  return (
    <div className="relative w-full overflow-hidden bg-transparent font-sans aspect-[2/1] md:aspect-[2.5/1] lg:aspect-[2/1]">
      <Image
                  ...
                  >
                    <foreignObject x={startPoint.x - 50} y={startPoint.y - 35} width="100" height="30" className="block">
                      <div className="flex h-full items-center justify-center">
                        <span
                          className={`rounded-md border border-[rgba(213,180,57,0.4)] bg-[rgba(6,10,22,0.9)] px-2 py-0.5 font-medium text-[#f3de6c] shadow-sm backdrop-blur-sm ${labelClassName}`}
                        >
                          {dot.start.label}
                        </span>
                      </div>
      ...
      <AnimatePresence>
        {hoveredLocation && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="absolute bottom-4 left-4 rounded-lg border border-[rgba(213,180,57,0.4)] bg-[rgba(6,10,22,0.9)] px-3 py-2 text-sm font-medium text-[#f3de6c] backdrop-blur-sm sm:hidden"
          >
            {hoveredLocation}
          </motion.div>
        )}
