...
function AnimatedCounter({
  ...
  return (
    ...
    >
      ...
      <motion.p
        className={cn(
          'text-gray-600 dark:text-gray-400 text-lg sm:text-xl md:text-2xl lg:text-3xl font-neue-montreal leading-relaxed px-1 sm:px-2 hyphens-auto wrap-break-word',
        )}
...
export default function StatsCount({
  ...
  return (
    <motion.section
      ref={containerRef}
      className={cn(
        'pt-0 pb-0 px-2 sm:px-4 md:px-8 w-full overflow-hidden',
        className,
      )}
      initial={{ opacity: 0 }}
      animate={isInView ? { opacity: 1 } : { opacity: 0 }}
    ...
    >
          ...
          <span className='hidden sm:inline'>
            {title.includes('WITH') ? (
              <>
                ...
                <span
                  className={cn(
                    'text-blue-600 dark:text-blue-400 font-neue-montreal-medium',
                  )}
                >
                  {title.split('WITH')[1]}
                </span>
      ...
      <div className={cn('w-full max-w-6xl mx-auto')}>
        <div
          className={cn(
            'flex flex-row items-stretch justify-between gap-6 sm:gap-8 lg:gap-12 w-full min-h-30 sm:min-h-35',
          )}
        >
          {stats.map((stat, index) => (
            ...
            >
              ...
              {index < stats.length - 1 && showDividers && (
                <motion.div
                  className={cn(
                    'absolute -right-1 sm:-right-2 lg:-right-4 top-1/2 transform -translate-y-1/2 h-28 sm:h-32 lg:h-40 w-px bg-gray-200 dark:bg-gray-700',
                  )}
